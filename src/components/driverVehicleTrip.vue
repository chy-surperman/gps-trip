...
<script>
	import {getRouteMsg,getVehicleMileage,getDriverVehicleTrip} from '@/api/gpsApi'
	...
	export default{
		...
		methods:{
			...
			queryTripDetail(){
				...
				getDriverVehicleTrip(param).then(resp => {
					if(resp.code == REQ_SUCCESS){
			...
			exportData(){
				let url = baseURL + "/report/down/driverVehicle?routeName=" + this.routeName + "&workdate=" + this.workdate + "&statisticsType=" + this.statisticsType
				axios({
					method:'get',
			        url,
			        responseType:'blob',
